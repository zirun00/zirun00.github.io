        ...
        if (!is_animate && !$(this).hasClass('active')) {
            ...
            if (id != '#home') {
                if (last_active.attr('href') == '#home') {
                    $('#container').animate({
                        'bottom': '50%',
                        'marginBottom': 0 - parseInt(parseInt(parseInt($('header').height()) + parseInt($('#content').height()) + 20) / 2)
                    }, 400);
...
$(function(){
    ...
    $('#metp').click(function(){
        ...
        $('#metp').css('display','none');
    });
    $('#home').click(function(){

        $('#metp').css('display','block');
    })
});
...
var oMetp= document.getElementById('metp');
...
for (var i = 0; i < aLi.length; i++) {
    ...
    aLi[i].style.left = -720+'px';
    aLi[i].style.top =-560+'px';
    aLi[i].style.margin = 0;
}
oMetp.onclick = function () {
    var i = 0;
    if(ok){return;}
    ok=true;
        ...
        ;(function(){
            ...
            timer = setInterval(function () {
                ...
                // console.log(Math.floor(aLi[aLi.length-1].offsetHeight));
                // if (Math.floor(aLi[0].offsetHeight)==100) {
                if (i==-1) {
                    console.log(11111);
                    clearInterval(timer);
                    setTimeout(function(){
...
var oDiv=document.getElementById('touch');
var iSpeedX=3;
var iSpeedY=4;
slide();
